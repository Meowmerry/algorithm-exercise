...
Input : func
Output : func ruturn 
      : func cloure will take input as agrument;
      ; 
*/

const memoize = (func) => {
  // declare cache variable assign to empty object
  const cache = {};
  return (input)=>{
    const val = JSON.stringify(input);
    if(!(val in cache)) cache[val] = func(input);
    return cache[val]
  }
};

const  call= (n) => n * 2
const memoizeFunc = memoize(call)
console.log(memoizeFunc(3)) // 6
console.log(memoizeFunc(4)) // 8
